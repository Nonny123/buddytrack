...
class MapsActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private val logListener = object : ValueEventListener {
        ...
        override fun onDataChange(dataSnapshot: DataSnapshot) {
            ...
            val bitmaps: Bitmap? = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(resources,R.drawable.android), 200, 200, true)

            // Check if datasnapshot exits
            if (dataSnapshot.exists()) {
                // Get data from database
                val userLocation = dataSnapshot.child("partner").getValue(UserLocation::class.java)
        ...
        locationCallback = object : LocationCallback() {
            ...
            override fun onLocationResult(locationResult: LocationResult) {
                if (locationResult.locations.isNotEmpty()) {
                    ...
                    databaseRef = Firebase.database.reference
                    val userLocation = UserLocation("Nonso", location.latitude, location.longitude)
                    //destruction
